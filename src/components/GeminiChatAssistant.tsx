...
export default function GeminiChatAssistant({ onNavigate }: GeminiChatAssistantProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 font-sans" id="floating-chat-container">
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-[350px] sm:w-[420px] max-w-[90vw] h-[550px] max-h-[80vh] bg-white border border-gray-100 rounded-2xl shadow-2xl flex flex-col justify-between overflow-hidden z-50 animate-in fade-in-50 slide-in-from-bottom-5 duration-300">
          ...
          <div className="p-4 bg-slate-950 text-white flex justify-between items-center border-b border-slate-800">
            <div className="flex items-center gap-2.5">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] text-gray-400">متصل الآن ومستعد لمساعدتك بالمنزل العصري 🌟</p>
              </div>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-slate-50 space-y-4">
            {messages.map((m) => {
              ...
              return (
                ...
                >
                  {/* Icon Avatar */}
                  <div className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center border text-[10px] font-bold ${isAi ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' : 'bg-slate-900 border-slate-800 text-white'}`}>
                    {isAi ? <Sparkles className="w-3.5 h-3.5" /> : <User className="w-3.5 h-3.5" />}
                  </div>

                  {/* Bubble text */}
                  <div className={`p-3 rounded-2xl text-xs leading-relaxed ${isAi ? 'bg-white text-slate-900 border border-gray-150/50 shadow-xs' : 'bg-slate-900 text-white font-medium'}`}>
                    {/* Render basic custom paragraph breaks with bolding since markdown library may be heavy */}
                    {m.text.split('\n\n').map((paragraph, pIdx) => (
                      <p key={pIdx} className={pIdx > 0 ? 'mt-2' : ''}>
                        {paragraph.split('**').map((chunk, cIdx) => 
                          cIdx % 2 === 1 ? <strong key={cIdx} className="text-amber-600 font-extrabold">{chunk}</strong> : chunk
                        )}
                      </p>
                    ))}
                  </div>
            ...
            {loading && (
              <div className="flex gap-2 max-w-[80%] mr-0 ml-auto">
                ...
                <div className="bg-white border border-gray-100 p-3.5 rounded-2xl flex items-center gap-1.5 shadow-xs">
          ...
          <div className="p-3 border-t border-gray-100 bg-white">
              ...
              <div className="mb-3.5 flex flex-wrap gap-1.5">
                {quickQuestions.map((q, i) => (
                  <button
                    ...
                    className="text-[10px] text-right font-bold text-slate-700 bg-slate-50 border border-slate-100 hover:bg-amber-500/10 hover:border-amber-500/30 px-2.5 py-1.5 rounded-lg transition-all cursor-pointer truncate max-w-full"
                  >
                    {q}
                  </button>
                ))}
            ...
            >
              <input
                ...
                disabled={loading}
                className="flex-1 bg-gray-50 border border-gray-150 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:bg-white focus:ring-1 focus:ring-amber-500 focus:border-amber-500 disabled:opacity-50"
              />
              <button
                type="submit"
                disabled={!input.trim() || loading}
                className="p-2.5 rounded-xl bg-slate-950 hover:bg-amber-500 text-amber-500 hover:text-slate-950 transition-colors disabled:opacity-40 disabled:hover:bg-slate-950 disabled:hover:text-amber-500 cursor-pointer"
                aria-label="إرسال"
              >
